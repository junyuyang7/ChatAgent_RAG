...
def run_async(cor: Future):
    ...
    try:
        ...
        loop = asyncio.new_event_loop() # 创建并返回一个新的事件循环对象。
    return loop.run_until_complete(cor) # 运行直到 cor ( Future 的实例 ) 被完成。


def iter_over_async(ait, loop=None):
    '''
    将异步生成器封装成同步生成器。使得它可以在同步代码中被调用和使用，而不需要使用 await 关键字来等待异步操作的结果。
    '''
    ait = ait.__aiter__()
...
def MakeFastAPIOffline(
    ...
    在离线模式下提供 API 文档和交互界面，以便在没有互联网连接的情况下查看和使用 API 文档。
    """
    ...
    ) # 生成文档界面的 HTML 内容，swagger_ui是编写API文档的规范。
...
def get_server_configs() -> Dict:
    ...
    # locals() 是一个内置函数，用于返回当前作用域中所有局部变量的字典。
...
def run_in_thread_pool(
    ...
    with ThreadPoolExecutor() as pool:
        for kwargs in params:
            thread = pool.submit(func, **kwargs) # Future 实例由 Executor.submit() 创建。
            tasks.append(thread)
