...
from server.db.repository.knowledge_file_repository import (
    ...
    count_files_from_db, list_files_from_db, get_file_detail, delete_docs_from_db,
    list_docs_from_db,
)
...
class KBService(ABC):
    '''
    基类：创建 / 增删改查数据库 
    保存向量库 / 创建知识库 / 删除向量库内容 / 添加文件 / 更新知识库 以及各种抽象方法接口
    '''
    def __init__(self,
...
class KBServiceFactory:
    '''数据库服务工厂：获取各类数据库的service（FAISS / PG / MILVUS / ZILLIZ）'''
    @staticmethod
...
class EmbeddingsFunAdapter(Embeddings):
    '''
    嵌入式模型适配器类，用于处理文本的嵌入式表示。
    （调用embedding_api中的函数使用huggingface上的编码器来编码文本）
    '''
