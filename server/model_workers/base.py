...
# 当你在模块中使用 from module import * 语句导入时，只会导入 __all__ 中列出的符号，而不会导入其他未在 __all__ 中列出的符号。这样可以控制模块的公开接口，避免不必要的符号被导入。
...
class ApiConfigParams(BaseModel):
    ...
    def validate_config(cls, v: Dict) -> Dict:
        '''该方法首先尝试从配置中获取与给定 worker_name 相关的配置信息，然后遍历模型类的所有字段，将配置中存在的字段值更新到输入数据中。最后返回更新后的数据字典。'''
...
class ApiModelWorker(BaseModelWorker):
    ...
    def __init__(
        ...
        **kwargs,
        ...
        # 恢复被fastchat覆盖的标准输出，将 sys.stdout 和 sys.stderr 重定向回其原始值。
        ...
        self.semaphore = asyncio.Semaphore(self.limit_worker_concurrency) # 创建信号量对象
        self.version = None
    ...
    def do_chat(self, params: ApiChatParams) -> Dict:
        '''
        执行Chat的方法，默认使用模块里面的chat函数。调用模型api进行对话
